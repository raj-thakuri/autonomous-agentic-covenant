...
*New in v2.0. Addresses AAC-DNH-0d (Trust Harm).*
...
*New in v2.0. Addresses the sub-agent spawning.*
...
*New in v2.0. Closes the conditional effectiveness gap in AAC-TB-29 against backdoor and poisoning attacks.*
...
**Tier 3 principles are optional implementation guidance — no maturity label applies. They support the control plane at any level but are not required for AAC compliance.**

**These are implementation patterns that support the governance control plane and matter for the system's integrity, performance, and maintainability. They are not enforced per-action by the control plane, but their absence may create conditions that make AAC compliance harder to sustain. Each includes a note on its governance relevance.**
...
*Governance relevance: External dependency failures are the primary trigger for DA-61 fallback actions. Without circuit breakers, a single downstream outage can produce cascading governance deadlock — the arbitration layer attempts actions that cannot complete, exhausts the liveness window, and falls back repeatedly. Circuit breakers contain the blast radius before it reaches the governance layer.*
...
A companion document set — the *AAC v2.0 Implementation Profile: L1 Foundational* and the *AAC v2.0 Implementation Profile: L2 and L3 Annexes* — provides structured templates for the deployment-specific parameterization required by AAC-OG-29. The L1 document covers all Foundational parameters; the L2/L3 document extends it for Mature and Optimizing deployments. The templates are not normative; the requirements in AAC-OG-29 are. Organizations may use any format that satisfies the declaration requirements.
...
- Governance control plane policy configurations
...
The AAC is an independent specification. The following are acknowledged for their influence on the framework's design, scope, and crosswalk coverage.
...
https://www.oreilly.com/library/view/risk-centric-threat-modeling/9780470500965
...
> Version 2.0 · April 2026 · 67 principles · Three enforcement tiers · 13 domains · 73 control identifiers.

> Evolved from: Thakuri, R. (2026). *The Autonomous Agentic Covenant: A Governance Framework for Autonomous and AI-Driven Systems* (Version 1.3). SSRN. https://dx.doi.org/10.2139/ssrn.6526238
>
> Version 1.3 remains the reference for the framework's ethical philosophy and governance vision.
